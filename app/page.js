...
const Home = () => {
  ...
  useEffect(() => {
    ...
    tl.from(createRef.current, {
      x: -100,
      opacity: 0,
    })
      .from(publishRef.current, {
        x: 100,
        opacity: 0,
      })
      .from(growRef.current, {
        opacity: 0,
        scale: 0.8,
      });
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <div className="fixed inset-0 bg-gradient-to-br from-purple-800/20 via-blue-900/20 to-green-800/30 animate-pulse"></div>

      {/* Dynamic cursor effect */}
      <div
        className="fixed w-80 h-80 bg-gradient-to-r from-blue-500/20 to-purple-700/40 rounded-full blur-3xl pointer-events-none z-0"
        style={{
      ...
      <span ref={createRef} className="block font-black text-white">
        Create.
      </span>
      <span ref={publishRef} className="block font-light italic text-purple-300">
        Publish.
      </span>
      <span
        ref={growRef}
        className="block font-black bg-gradient-to-r from-blue-400 via-purple-400 to-green-400 bg-clip-text text-transparent"
      >
        Grow.
      </span>
    </h1>
      ...
      {/* Footer */}
      <footer className="relative z-10 border-t py-12 px-4 sm:px-6 min-h-24">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-muted-foreground">
           All Rights are reserved &copy; {new Date().getFullYear()}
            <span className="">{"  "}by {"  "}
              <span 
            className= " inline-block animate-caret-blink text-md  sm:text-3xl bg-gradient-to-r from-purple-500 via-blue-400 to-green-300 bg-clip-text text-transparent"> 
